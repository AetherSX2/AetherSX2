...
bool GSTextureCache::m_preload_texture = false;
	...
	, m_palette_map(r)
{
	...
	m_preload_texture = theApp.GetConfigB("preload_texture");
...
void GSTextureCache::Source::Update(const GSVector4i& rect, int layer)
{
	...
	GSVector4i r;
	if (m_preload_texture)
	{
		r = GSVector4i(0, 0, std::max(tw >> layer, 1), std::max(th >> layer, 1));
		m_complete = true;
	}
	else
	{
		r = rect.ralign<Align_Outside>(bs);
	...
	if (blocks > 0)
	{
		...
		if (m_preload_texture)
			Write(GSVector4i(0, 0, std::max(tw >> layer, 1), std::max(th >> layer, 1)), layer);
...
static u32 HashTexture(u8* buff, int tw, int th, int pitch, bool palette)
{
	u32 hw = palette ? static_cast<u32>(std::max(tw >> 2, 1)) : static_cast<u32>(tw);
	u32 crc = 0;
	if ((hw & 1u) != 0)
	{
		for (u32 y = 0; y < static_cast<u32>(th); y++)
		{
			const u32* row = (u32*)(buff + y * static_cast<u32>(pitch));
			for (u32 x = 0; x < hw; x++)
			{
#ifdef _M_ARM64
				crc = __crc32w(crc, row[x]);
#else
				crc = _mm_crc32_u32(crc, row[x]);
#endif
			}
		}
	}
	else
	{
		...
		for (u32 y = 0; y < static_cast<u32>(th); y++)
		{
			const u64* row = (u64*)(buff + y * static_cast<u32>(pitch));
			for (u32 x = 0; x < hw; x++)
			{
#ifdef _M_ARM64
				crc = __crc32d(crc, row[x]);
#else
				crc = _mm_crc32_u64(crc, row[x]);
#endif
			}
		}
	}
...
void GSTextureCache::Source::Flush(uint32 count, int layer)
{
	...
	for (uint32 i = 0; i < count; i++)
	{
		...
		if (m_preload_texture)
		{
			...
			const u32 hash = HashTexture(buff, std::max(tw >> layer, 1), std::max(th >> layer, 1), pitch, m_palette != nullptr);
			if (m_layer_hash[layer] != hash)
			{
